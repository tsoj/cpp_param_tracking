...
def get_git_log_range(file_path: str, start_line: int, end_line: int, max_commits: int) -> List[Tuple[str, str]]:
    ...
    try:
        ...
        for line in lines:
            if line and not line.startswith('diff') and not line.startswith('---') and \
...
def track_parameters(file_path: str, start_line: int, end_line: int, max_commits: int) -> Dict:
...
def create_annotated_code(original_code: str, statements: List[Dict], param_map: Dict) -> str:
        ...
        for param_idx, number in enumerate(stmt['numbers']):
            ...
            if key in param_map:
                ...
                if match:
                    ...
                    annotated_stmt = annotated_stmt[:pos] + param_id + annotated_stmt[pos + len(number):]
...
def main():
    ...
    parser.add_argument("--end-line", type=int, required=True, help="End line number (inclusive)")
    parser.add_argument("--max-commits", type=int, default=10, help="Maximum number of commits to track")

    args = parser.parse_args()

    result = track_parameters(args.file, args.start_line, args.end_line, args.max_commits)
    ...
    if result:
        ...
        print("=" * 80)
        for param_id in sorted(param_history.keys(), key=lambda x: int(x.split('_')[1])):
